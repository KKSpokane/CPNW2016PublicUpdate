<?php ?>

<!--[if lt IE 9]>
    <div class="ieblock">
      Your Browser is out of date! Please upgrade to the newest version of <a href="http://www.google.com/chrome/">Chrome</a>, <a href="https://www.apple.com/safari/">Safari</a> or <a href="https://www.mozilla.org/en-US/firefox/new/">Firefox</a>.
    </div>
<![endif]-->


<header class="hbg">
      <div class="row">
        <div class="small-12 columns">
          <div class="logo">
            <a href="index.php">
              <img src="images/logo.svg" alt="CPNW" />
            </a>
          </div>
          <div class="menu"><a href=""><span class="icon"></span></a></div>
          <div class="nav-wrapper">
            <a class="mobile-close" href="#"></a>
            <div class="mobile">

              <nav class="utility">
                <ul>
                    <li><a href="#">Register</a></li>
                  <li><a href="dashboard.php">My Dashboard</a></li>
                  <li><a href="#">Login</a></li>
                </ul>
              </nav>
                <nav class="primary">
                <ul>
                  <li class="nav-dropdown wide"><a href="about.php">About Us</a>
                    <ul>
                      <li><a href="#">Healthcare Partners</a></li>
                      <li><a href="#">Participating Programs</a></li>
                      <li><a href="#">Nursing Placement</a></li>
                    </ul>
                  </li>
                  <li><a href="services.php">Our Services</a></li>
                  <li><a href="new-faq.php">FAQ</a></li>
                  <li class="nav-dropdown edge"><a href="get_started.php">Get Started</a>
                    <ul>
                      <li><a href="#">Schools</a></li>
                      <li><a href="#">Students</a></li>
                      <li><a href="#">Healthcare</a></li>
                    </ul>
                  </li>
                </ul>
              </nav>
            </div>
          </div>
        </div>
      </div>
    </header>